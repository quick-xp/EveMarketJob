...
/***
 * EveMarketDataと最終報告日付一覧を読み込み、Regionid,Systemid,typeIdをキーとし、各システム毎の品目別売上を求める
 * 
 * @author chato
 */
public class AnalyzeSellOrderPriceInEachSystem extends Configured implements
		Tool {
				...
				if (newest.getBid().equals("0")) {
					...
					if (newest.getReportedtime().getTime() < stationLastReport
						...
						if (newest.getReportedtime().getTime() + 6 * 60 * 1000
							...
							sellPrice += newest.getPrice() * sellCount;
							System.out.println("order_sold_out : "
									+ newest.getOrderid());
						}
					}else{
	...
	public int run(String[] args) throws Exception {
		...
		conf.setJobName("AnalyzeSellOrderPriceInEachSystem");
